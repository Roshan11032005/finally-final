...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-common.git \
    && cmake -S aws-c-common -B aws-c-common/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-checksums.git \
    && cmake -S aws-checksums -B aws-checksums/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
    && cmake --build aws-checksums/build --config Release -j \
    && cmake --install aws-checksums/build

# s2n-tls (TLS backend required by aws-c-io)
RUN git clone --depth=1 https://github.com/aws/s2n-tls.git \
    && cmake -S s2n-tls -B s2n-tls/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON -DS2N_NO_PQ=ON \
    && cmake --build s2n-tls/build --config Release -j \
    && cmake --install s2n-tls/build

# aws-c-cal (before aws-c-io)
RUN git clone --depth=1 https://github.com/awslabs/aws-c-cal.git \
    && cmake -S aws-c-cal -B aws-c-cal/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-io.git \
    && cmake -S aws-c-io -B aws-c-io/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-compression.git \
    && cmake -S aws-c-compression -B aws-c-compression/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-http.git \
    && cmake -S aws-c-http -B aws-c-http/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-sdkutils.git \
    && cmake -S aws-c-sdkutils -B aws-c-sdkutils/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-auth.git \
    && cmake -S aws-c-auth -B aws-c-auth/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
...
RUN git clone --depth=1 https://github.com/awslabs/aws-c-s3.git \
    && cmake -S aws-c-s3 -B aws-c-s3/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
    && cmake --build aws-c-s3/build --config Release -j \
    && cmake --install aws-c-s3/build

# Make sure loader finds shared libs
...
# Run the app (pass env vars via docker run)
